...
def _prompt_path(puzzle: dict[str, Any]) -> Path:
    return PUBLIC_DIR / "prompts" / f"{puzzle['subset_rank']:03d}_{puzzle['id']}.md"


def _puzzle_file_path(puzzle: dict[str, Any]) -> Path:
    return PUBLIC_DIR / "puzzles" / f"{puzzle['subset_rank']:03d}_{puzzle['id']}.json"
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    if manifest.get("benchmark_id") != "ppbench_repr50":
        errors.append("manifest benchmark_id must be ppbench_repr50")
    ...
    if len(puzzles) != 50:
        errors.append(f"expected 50 public puzzles, found {len(puzzles)}")
    ...
    if sorted(type_counts.values()) != ([2] * 10 + [3] * 10):
        errors.append(f"type counts must be ten 2s and ten 3s; got {json.dumps(type_counts, sort_keys=True)}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PPBench repr50 bundle helper")
    ...
    show_parser.add_argument("puzzle", help="One-based subset rank or puzzle id")
