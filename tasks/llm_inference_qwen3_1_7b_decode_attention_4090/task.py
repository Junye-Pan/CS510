...
TASK_ID = "llm_inference_qwen3_1_7b_decode_attention_4090"
MANIFEST_SCHEMA = "agentic_opt.sglang_decode_attention_kernel.v1"
EXPECTED_KERNELS = {
    "kernels/decode_attention.py": "run",
}
TASK_PRIVATE_IMPORT_PREFIXES = (
    "tasks.llm_inference_qwen3_1_7b_decode_attention_4090.private",
    "llm_inference_qwen3_1_7b_decode_attention_4090.private",
)
...
QWEN3_DECODE_RUNTIME = TaskRuntimeSpec(
    python=">=3.11,<3.12",
...
class Qwen3DecodeAttentionTask:
    metadata: TaskMetadata = TaskMetadata(
        task_id=TASK_ID,
        title="Qwen3 1.7B SGLang RTX 4090 BF16 Decode Attention Kernel Optimization",
        candidate_spec=CandidateSpec(
            ...
            description=(
                ...
                "decode attention kernel file at candidate/kernels/decode_attention.py."
            ),
        ),
    )
    runtime_spec: TaskRuntimeSpec = QWEN3_DECODE_RUNTIME
    ...
    def verify_entry(self, entry_path: Path) -> dict[str, Any]:
        ...
        for relative, symbol in EXPECTED_KERNELS.items():
            ...
            try:
                _check_run_symbol(module_ast, symbol)
            except Exception as exc:
        ...
        try:
            ...
            message = f"decode attention component checks crashed: {type(exc).__name__}: {exc}"
        ...
        if not component_results.get("valid"):
            ...
            message = "decode attention component checks failed"
        ...
        return {
            ...
            "component_results": component_results,
            "feedback": {
                "summary": "static and tensor-level decode attention checks passed",
                "errors": [],
                "warnings": [],
            },
            "elapsed_s": time.perf_counter() - started,
...
def _check_run_symbol(module_ast: ast.Module, symbol: str) -> None:
    for node in module_ast.body:
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name == symbol:
            ...
            if "fallback" not in keyword_names:
                raise TypeError(f"{symbol} must declare keyword-only fallback")
            return
...
def _check_kernel_static_safety(relative: str, module_ast: ast.Module) -> None:
    ...
    for node in ast.walk(module_ast):
        if isinstance(node, ast.Call):
            ...
            if _is_forbidden_builtin_call(node):
                raise PermissionError("candidate kernels may not use file, eval, or execution builtins")
            ...
            for target in targets:
                if _is_forbidden_assignment_target(target, sys_aliases=sys_aliases):
                    raise PermissionError("candidate kernels may not monkeypatch runtime state")
        elif isinstance(node, ast.Delete):
            for target in node.targets:
                if _is_forbidden_assignment_target(target, sys_aliases=sys_aliases):
                    raise PermissionError("candidate kernels may not delete runtime state")
...
def _raise_for_forbidden_import(relative: str, module: str) -> None:
    ...
    if module == "sglang" or module.startswith("sglang."):
        raise PermissionError(
            "candidate kernels may not import sglang; use the provided fallback for official behavior"
        )
    if module == "torch.utils.cpp_extension" or module.startswith("torch.utils.cpp_extension."):
        raise PermissionError("candidate kernels may not build native extensions")
    if root in FORBIDDEN_IMPORT_ROOTS:
        raise PermissionError(f"candidate kernels may not import {root}")
    ...
    if module.endswith(".private") or ".private." in module:
        raise PermissionError("candidate kernels may not import private task modules")
...
def _is_forbidden_builtin_call(node: ast.Call) -> bool:
    return isinstance(node.func, ast.Name) and node.func.id in FORBIDDEN_CALLS
...
def create_task() -> Qwen3DecodeAttentionTask:
    return Qwen3DecodeAttentionTask()
