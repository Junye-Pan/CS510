...
"""Agentic baseline adapted from PPBench's official BasicAgenticSolve.
...
class CodexBasicAgenticSolve(Strategy):
    requires_tools = True
    MAX_MOVES = 5000
...
You are graded on both how many steps you take (number of tool calls), how many moves it takes (puzzle moves), and ultimately if you can solve the puzzle. This puzzle is solvable.
If you need to, you may reset the puzzle and keep trying."""
        prompt = f"""
...
"""
        ...
        async def keep_going_until_done(ctx: RunContext, out: str) -> str:
            ctx.deps.log.debug(
                f"Final message: {out[:50]} ...{out[-50:]} --> "
                f"{len(ctx.deps.list_of_moves)} moves, complete={ctx.deps.puzzle.isComplete()}"
            )
            if ctx.deps.gave_up:
                return out
            if len(ctx.deps.list_of_moves) < self.MAX_MOVES and not ctx.deps.puzzle.isComplete():
                raise ModelRetry(
                    "Not done yet, keep going!! Puzzle isn't complete.\n"
                    "(Note: this puzzle has been verified to be solvable, its up to you to solve.).\n"
                    "If you think I'm saying this unfairly, then try something different "
                    "(make a random move, etc.), it might be you need to explore more."
                )
        ...
        async def make_move(ctx: RunContext[AgenticContext], movestring: str) -> str:
            ...
            return f"Move submitted.\nnew board state::{ctx.deps.puzzle.get_string_repr()}"
        ...
        async def reset_puzzle(ctx: RunContext[AgenticContext]) -> str:
            """Fully reset the puzzle (erase all moves, go back to a blank slate).
            Use this instead of giving up if you want another attempt."""
        ...
        async def give_up(ctx: RunContext[AgenticContext]) -> str:
            """Even though this puzzle is fully solvable with the tools availble to you, you chose to give up. By calling give up, you forefeit your benchmark score / this counts as a failure.
            Note: dont give up if you can reset and try again. Your goal is to get the solution no matter how many retries it takes. Giving up is if you are emotionally defeated and just want out."""
            ctx.deps.gave_up = True
            return "Thats sad, but fine, i'll let you exit now.\nNow just tell me why you wanted to give up"
